<main class="pt-8 pb-16 lg:pt-16 lg:pb-40 bg-white dark:bg-gray-900 antialiased">
        <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
            <article class="mx-auto w-full max-w-4xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
                ...
                <header class="my-4 lg:mb-6 not-format">
                    <address class="flex items-center mb-6 not-italic">
                        <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                            <img class="mr-4 w-16 h-16 rounded-full" src="{{ Auth::user()->avatar ? asset('storage/'. Auth::user()->avatar) : asset('storage/'. 'img/default.png') }}" alt="{{ Auth::user()->name }}">
                            <div>
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-3 sm:space-x-4">
                            ...
                            <button type="button" data-modal-target="deleteModal-{{ $post->id }}" data-modal-toggle="deleteModal-{{ $post->id }}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400 cursor-pointer">
                                <svg class="mr-1 -ml-1 w-5 h-5" viewbox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                ...
                <!-- sanitasi tanpa pengeceken keamanan -->
                <div>{!! $post['body'] !!}</div>
                {{-- <div>{{ $post['body'] }}</div> --}}
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <div class="flex justify-center items-center space-x-4">
                        ...
                        <form action="/dashboard/{{ $post->slug }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
